...
## Patron: Tiempo Real (Realtime) — evitar el fallo silencioso mas comun

Cuando el requisito es "que se vea reflejado sin recargar" (un admin habilita algo y otros
usuarios lo ven en vivo, un chat/foro que se actualiza solo), hay DOS pasos obligatorios y
solo uno de ellos se hace en el codigo del cliente. Olvidar el segundo produce un bug real:
el codigo compila y no marca ningun error, pero nadie recibe eventos nunca.

### Paso 1 (cliente): suscribirse a postgres_changes

```typescript
useEffect(() => {
  const channel = supabase
    .channel('nombre_unico_del_canal')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'nombre_tabla' }, () => {
      recargarDatos() // el patron mas simple y robusto: volver a pedir todo, no parchear estado local
    })
    .subscribe()
...
### Paso 2 (base de datos): agregar la tabla a la publicacion `supabase_realtime`

**Esto NO pasa solo por crear la tabla o por habilitar RLS.** Si la tabla no esta en la
publicacion, el `.subscribe()` del cliente conecta sin error (`status: SUBSCRIBED`) pero
JAMAS recibe un evento — no hay excepcion, no hay warning, simplemente nunca llega nada.
...
Alternativa sin SQL: Dashboard → Database → Replication → activar el switch de la tabla.

Si no tienes acceso de DDL (ver seccion de credenciales), entrega este SQL al usuario para
que lo corra el en el SQL Editor — no lo puedes activar tu solo sin ese acceso.

### Como verificar que de verdad funciona (no asumir)

No confies en que "deberia funcionar" porque el codigo se ve correcto. Prueba real minima:
suscribete con un script aparte, dispara un INSERT/UPDATE de prueba, y confirma que el
callback SI se ejecuta, antes de dar el feature por terminado:

```javascript
const canal = supabase.channel('verificacion')
  .on('postgres_changes', { event: '*', schema: 'public', table: 'nombre_tabla' }, (payload) => {
    console.log('EVENTO RECIBIDO:', payload.eventType) // si esto nunca imprime, falta el paso 2
  })
  .subscribe((status) => console.log('estado suscripcion:', status))

// ...dispara aqui un insert/update de prueba contra la misma tabla y espera unos segundos...
```

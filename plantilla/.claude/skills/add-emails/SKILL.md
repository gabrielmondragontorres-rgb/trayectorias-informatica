...
3. **Pregunta primero: ¿el usuario tiene un dominio propio que pueda verificar en Resend?**
   Si la respuesta es no, NO ejecutes el Golden Path de abajo — ve a la sección
   "Sin dominio propio verificado" antes de instalar nada.

## Sin dominio propio verificado (decisión previa obligatoria)

Aprendizaje real de producción: sin un dominio verificado, Resend solo permite enviar al
correo con el que se creó la cuenta — **no puede entregar a destinatarios reales**. La
alternativa "fácil" de usar Gmail SMTP (nodemailer con una cuenta Gmail dedicada) parece
funcionar (las credenciales son válidas, el login SMTP no da error) pero en la práctica el
correo cae en spam o se descarta silenciosamente casi siempre, porque los proveedores
grandes penalizan el envío automatizado desde IPs de servidores serverless (Vercel, Lambda,
etc.) aunque la autenticación sea correcta. Esto **no se nota en pruebas locales** — el
`transporter.verify()` regresa éxito, y el fallo solo aparece cuando un destinatario real
nunca recibe nada.

**No implementes envío automático de correo si no hay dominio propio.** En su lugar:

1. **Botón de contacto flotante** (patrón validado): un botón fijo en pantalla que, al hacer
   clic, abre directamente `https://mail.google.com/mail/?view=cm&fs=1&to=CORREO&su=ASUNTO`
   (compose web de Gmail) — no depende de ningún cliente de correo predeterminado del sistema
   operativo (evita el problema de que `mailto:` abra Outlook u otro cliente no deseado), no
   requiere backend, y no falla nunca porque no hay entrega automática que pueda fallar. Ver
   implementación de referencia en el proyecto que originó este aprendizaje:
   `src/shared/components/ContactoFlotante.tsx`.
2. **Mensaje de bienvenida in-app en vez de correo de notificación**: si la necesidad real es
   avisarle a alguien que su cuenta fue aprobada/activada, no intentes notificarlo por un canal
   externo poco confiable — muéstraselo dentro de la propia app la primera vez que inicia
   sesión. Detecta "primera vez" con un contador que la acción de aprobación del admin NO
   modifica (ej. `total_conexiones === 0` antes de esa conexión), para no depender de
   localStorage ni de temporizadores.
3. Si más adelante el usuario adquiere un dominio, entonces sí aplica el Golden Path completo
   de este skill con Resend.
